...
/// <summary>The base class and entry point of AWS Lambda Functions which perform an action.</summary>
    ...
    where THandler : class, IHandler<TIn>
{
    ...
    public async Task HandleAsync(TIn input, ILambdaContext context)
    {
        ...
        await using var warningRegistrationScope = warningRegistration.ConfigureAwait(false);
        try
        {
            var handler = scope.ServiceProvider.GetHandler<TIn>();
            await handler.HandleAsync(input, context, cts.Token).ConfigureAwait(false);
        }
